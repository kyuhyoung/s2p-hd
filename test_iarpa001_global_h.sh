...
# IARPA_001: per-tile vs global-H benchmark on FoundationStereo with EO-NeRF GT.
# Picks the 02APR15 along-track stereo pair (134718 / 134802, 44s apart),
# crops a 4000x4000 ROI centered on the image, and runs 4 combinations
# (tile=1000 per-tile, tile=1000 global-H, tile=600 per-tile, tile=600 global-H).
#
# Run inside docker container:
#   bash test_iarpa001_global_h.sh
#
# Expects these already present on the host (rsync from 241):
#   /data/kevin_workspace/dataset_stereo/satellite/eonerf_dataset/image_rpc/IARPA_001/
#   /data/kevin_workspace/dataset_stereo/satellite/eonerf_dataset/truth/IARPA_001/

: "${CUDA_VISIBLE_DEVICES:=7}"
export CUDA_VISIBLE_DEVICES

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LOGFILE="${SCRIPT_DIR}/test_iarpa001_global_h.log"
DATA_DIR=/data/satellite/eonerf_dataset/image_rpc/IARPA_001
TRUTH_DIR=/data/satellite/eonerf_dataset/truth/IARPA_001

# Stereo pair (same day, 44s apart)
IMG_L=02APR15WV031000015APR02134718-M1BS-500497282050_01_P001_________GA_E0AAAAAAKAAI0.tif
RPC_L=02APR15WV031000015APR02134718-M1BS-500497282050_01_P001_________GA_E0AAAAAAKAAI0.RPB
IMG_R=02APR15WV031000015APR02134802-M1BS-500276959010_02_P001_________GA_E0AAAAAAKAAH0.tif
RPC_R=02APR15WV031000015APR02134802-M1BS-500276959010_02_P001_________GA_E0AAAAAAKAAH0.RPB
...
if [ -z "$GT" ]; then
    GT=$(ls "${TRUTH_DIR}"/*DSM.tif 2>/dev/null | head -1)
fi
if [ -z "$GT" ]; then
    log "${RED}no GT DSM found in ${TRUTH_DIR}${NC}"
    ls "${TRUTH_DIR}/" | sed 's/^/  /' | while read l; do log "$l"; done
    exit 1
fi
log "GT: ${GT}"

# Force-reinstall host code
...
# Determine image size -> center 4000x4000 ROI
...
ROI_W=4000
ROI_H=4000
if [ "$IMG_W" -lt "$ROI_W" ]; then ROI_W=$IMG_W; fi
if [ "$IMG_H" -lt "$ROI_H" ]; then ROI_H=$IMG_H; fi
ROI_X=$(( (IMG_W - ROI_W) / 2 ))
ROI_Y=$(( (IMG_H - ROI_H) / 2 ))
...
for TS in 1000 600; do
    for MODE in pertile globalH; do
        OUT_REL="./iarpa001_ts${TS}_${MODE}"
        CFG="config_iarpa001_ts${TS}_${MODE}.json"
        USE_GLOBAL=false
...
GT = '${GT}'
runs = [
    ('tile=1000 per-tile', './iarpa001_ts1000_pertile/dsm.tif'),
    ('tile=1000 global-H', './iarpa001_ts1000_globalH/dsm.tif'),
    ('tile=600  per-tile', './iarpa001_ts600_pertile/dsm.tif'),
    ('tile=600  global-H', './iarpa001_ts600_globalH/dsm.tif'),
]
...
log ""
log "${GREEN}All done. DSMs in ${DATA_DIR}/iarpa001_ts*_*/dsm.tif${NC}"
