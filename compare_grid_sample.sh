...
# Compare the cuDNN and PyTorch-native bilinear interpolation kernels at the
# operation level: both run on the same input tensor at every call.
#
#   bash compare_grid_sample.sh          # default 1536
#   SIZE=1024 bash compare_grid_sample.sh
...
LOGFILE="${SCRIPT_DIR}/compare_grid_sample.log"
...
log "${GREEN}=== bilinear interpolation: cuDNN vs PyTorch native (size=${SIZE}) ===${NC}"
log ""
...
docker run --rm \
    ...
    bash -c "
        ...
        python3 -u /workspace/tools/compare_grid_sample.py --size ${SIZE} \
