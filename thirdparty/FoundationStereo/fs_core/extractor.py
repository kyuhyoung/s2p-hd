...
class ContextNetDino(MultiBasicEncoder):
    def __init__(self, args, output_dim=[128], norm_fn="batch", downsample=3):
        ...
        vit_dim = (
            DepthAnythingFeature.model_configs[self.args.vit_size]["features"] // 2
        )
...
class DepthAnythingFeature(nn.Module):
    ...
    def forward(self, x):
        ...
        out, path_1, path_2, path_3, path_4, disp = (
            self.depth_anything.depth_head.forward(
...
class Feature(nn.Module):
    def __init__(self, args):
        ...
        model = timm.create_model(
            "edgenext_small", pretrained=True, features_only=False
        )
    ...
    def forward(self, x):
        ...
        vit_feat = F.interpolate(
            vit_feat, size=(H // 4, W // 4), mode="bilinear", align_corners=True
        )
        x = self.stem(x)
        x4 = self.stages[0](x)
        x8 = self.stages[1](x4)
