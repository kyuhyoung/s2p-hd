...
from fs_core.update import *
from fs_core.extractor import *
from fs_core.geometry import Combined_Geo_Encoding_Volume
from fs_core.submodule import *
from fs_core.utils.utils import *
from Utils import *
...
class FoundationStereo(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, args):
        ...
        self.cost_agg = hourglass(
            cfg=self.args, in_channels=volume_dim, feat_dims=self.feature.d_out
        )
        ...
        dx = torch.linspace(-r, r, 2 * r + 1, requires_grad=False).reshape(
            1, 1, 2 * r + 1, 1
        )
        self.dx = dx
    ...
    def forward(
        ...
        with autocast(enabled=self.args.mixed_precision):
            ...
            prob = F.softmax(
                self.classifier(comb_volume).squeeze(1), dim=1
            )  # (B, max_disp, H, W)
            if init_disp is None:
                init_disp = disparity_regression(
                    prob, self.args.max_disp // 4
                )  # Weighted  sum of disparity

            cnet_list = self.cnet(
                image1, vit_feat=vit_feat, num_layers=self.args.n_gru_layers
            )  # (1/4, 1/8, 1/16)
            cnet_list = list(cnet_list)
            net_list = [torch.tanh(x[0]) for x in cnet_list]  # Hidden information
            inp_list = [
                torch.relu(x[1]) for x in cnet_list
            ]  # Context information list of pyramid levels
        ...
        coords = (
            torch.arange(w, dtype=torch.float, device=init_disp.device)
            .reshape(1, 1, w, 1)
            .repeat(b, h, 1, 1)
    ...
    def run_hierachical(
        ...
        img1_small = F.interpolate(
            image1, scale_factor=small_ratio, align_corners=False, mode="bilinear"
        )
        img2_small = F.interpolate(
            image2, scale_factor=small_ratio, align_corners=False, mode="bilinear"
        )
        ...
        disp_small = self.forward(
            img1_small, img2_small, test_mode=True, iters=iters, low_memory=low_memory
        )
        disp_small = padder.unpad(disp_small.float())
        disp_small_up = (
            F.interpolate(disp_small, size=(H, W), mode="bilinear", align_corners=True)
            * 1
            / small_ratio
        )
        ...
        init_disp = (
            F.interpolate(
                disp_small_up, scale_factor=0.25, mode="bilinear", align_corners=True
            )
            * 0.25
        )  # Init disp will be 1/4
        disp = self.forward(
            image1,
            image2,
            iters=iters,
