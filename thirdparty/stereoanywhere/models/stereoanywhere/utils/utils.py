...
def normalize(x, eps=1e-4):
    ...
    for i in range(len(x)):
        _min = -F.max_pool2d(
            -x[i], (x[i].size(2), x[i].size(3)), stride=1, padding=0
        ).detach()
...
def estimate_normals(depth, normal_gain):
    xy_gradients = -spatial_gradient(
        normal_gain * depth, mode="diff", order=1, normalized=False
    ).squeeze(
        1
    )  # B 2 H W
    normals = torch.cat(
        [xy_gradients, torch.ones_like(xy_gradients[:, 0:1])], 1
    )  # B 3 H W
...
def estimate_gradient_magnitude(depth, gradient_gain):
    xy_gradients = spatial_gradient(
        depth * gradient_gain, mode="diff", order=1, normalized=False
    ).squeeze(
        1
    )  # B 2 H W
...
def estimate_left_disparity(corr_volume, vol_pad=[0, 0]):
    ...
    disp_values = torch.arange(
        0, W3, dtype=corr_volume.dtype, device=corr_volume.device
    )
    disp_values_left = disp_values.view(1, 1, 1, -1).repeat(B, 1, 1, 1)  # B 1 1 W3

    mycoords_y, mycoords_x = torch.meshgrid(
        torch.arange(H, dtype=corr_volume.dtype, device=corr_volume.device),
        torch.arange(W2, dtype=corr_volume.dtype, device=corr_volume.device),
        indexing="ij",
    )
    ...
    prob_left = F.softmax(
        corr_volume.squeeze(1) * mask, dim=3
    )  # B, 1, H, W2, W3 -> B, H, W2, W3
...
def estimate_right_disparity(corr_volume, vol_pad=[0, 0]):
    ...
    disp_values = torch.arange(
        0, W2, dtype=corr_volume.dtype, device=corr_volume.device
    )
    disp_values_right = disp_values.view(1, 1, -1, 1).repeat(B, 1, 1, 1)  # B 1 W2 1

    mycoords_y, mycoords_x = torch.meshgrid(
        torch.arange(H, dtype=corr_volume.dtype, device=corr_volume.device),
        torch.arange(W3, dtype=corr_volume.dtype, device=corr_volume.device),
        indexing="ij",
    )
    ...
    prob_right = F.softmax(
        corr_volume.squeeze(1) * mask, dim=2
    )  # B, 1, H, W2, W3 -> B, H, W2, W3
...
def estimate_left_confidence(corr_volume, logsumexp_eps=1e-3):
    ...
    conf_left = -torch.sum(
        prob_left * torch.log2(prob_left + 1e-6), dim=3, keepdim=False
    ) / math.log2(
        W3
    )  # B H W2
...
def estimate_right_confidence(corr_volume, logsumexp_eps=1e-3):
    ...
    prob_right = F.softmax(
        corr_volume.squeeze(1), dim=2
    )  # B, 1, H, W2, W3 -> B, H, W2, W3
    # conf_right = logsumexp_eps * torch.logsumexp(prob_right/logsumexp_eps, dim=2, keepdim=False) # B H W3
    # Alternative based on information entropy
    conf_right = -torch.sum(
        prob_right * torch.log2(prob_right + 1e-6), dim=2, keepdim=False
    ) / math.log2(
        W2
    )  # B H W3
...
def disp_warping(disp, img, right_disp=False):
    ...
    mycoords_y, mycoords_x = torch.meshgrid(
        torch.arange(H, dtype=disp.dtype, device=disp.device),
        torch.arange(W, dtype=disp.dtype, device=disp.device),
        indexing="ij",
    )
    ...
    if right_disp:
        grid = (
            ...
            * torch.cat(
                [
                    (mycoords_x + disp.squeeze(1)).unsqueeze(-1) / W,
                    mycoords_y.unsqueeze(-1) / H,
                ],
                -1,
            )
            - 1
...
def softlrc(disp2, disp3, lrc_th=1.0):
    ...
    softlrc_disp2 = (
        F.softplus(-torch.abs(disp2 - warped_disp3) + lrc_th) / div_const
    )  # lrc weights in (0,1)  #B 1 H W
    softlrc_disp3 = (
        F.softplus(-torch.abs(disp3 - warped_disp2) + lrc_th) / div_const
    )  # lrc weights in (0,1) #B 1 H W
...
def gauss_corr_volume_naive(disp_left, gauss_k=10, gauss_c=1):
    ...
    gauss_center = mycoords_x.unsqueeze(1).unsqueeze(4) - disp_left.unsqueeze(
        4
    )  # B 1 H/4 W/4 1
    gauss_corr_left = gauss_center - disp_values_left.unsqueeze(1)  # B 1 H/4 W/4 W/4
    gauss_corr_left = gauss_k * torch.exp(
        -(gauss_corr_left**2) / (2 * gauss_c**2)
    )  # B 1 H/4 W/4 W/4
...
def truncate_corr_volume_v2(disp_left, conf_left, conf_th=0.5, attenuation_gain=0.1):
    ...
    truncate_center = mycoords_x.unsqueeze(1).unsqueeze(4) - disp_left.unsqueeze(
        4
    )  # B 1 H/4 W/4 1
    truncate_corr_left = truncate_center - disp_values_left.unsqueeze(
        1
    )  # B 1 H/4 W/4 W/4

    # I'm expecting the pixel corrispondece to be in the left side of truncate_center
    # Sigmoid sign tested with a simple example
    truncate_corr_left = 1 * (1 - conf_left) + (conf_left) * (
        F.sigmoid(truncate_corr_left) * (1 - attenuation_gain) + attenuation_gain
    )
...
def handcrafted_mirror_detector(
    stereo_disp, mono_disp, stereo_conf, mono_conf, conf_th=0.5, step_gain=20
):
...
def corr(normals_left, normals_right):
    ...
    return corr.squeeze(3).unsqueeze(
        1
    )  # torch.Size([B, H, W1, 1, W2]) -> B, 1, H, W1, W2
...
def weighted_lsq(mde, disp, conf, min_quantile=0.2, max_quantile=0.9):
    ...
    # Weighted LSQ
    mde, disp, conf = (
        mde.reshape(B, -1).float(),
        disp.reshape(B, -1).float(),
        conf.reshape(B, -1).float(),
    )
    ...
    for b in range(B):
        ...
        _scale_shift = torch.linalg.lstsq(A_matrix, B_matrix)[0].squeeze(
            2
        )  # 1 x 2 x 1 -> 1 x 2,
        scale_shift[b] = _scale_shift.squeeze(0)

    return scale_shift[:, 0:1].reshape(B, 1, 1, 1).to(mde_dtype), scale_shift[
        :, 1:2
    ].reshape(B, 1, 1, 1).to(mde_dtype)
...
def naive_scale_shift(mde, disp, conf, conf_th=0.5):
    ...
    return scale_and_shift_values[:, 0:1].reshape(B, 1, 1, 1), scale_and_shift_values[
        :, 1:2
    ].reshape(B, 1, 1, 1)
