...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[], predict_confidence=True):
        ...
        if self.predict_confidence:
            self.conf_head = SigmoidUpdateHead(
                hidden_dims[2], hidden_dim=256, output_dim=1
            )
...
class BasicMultiUpdateScalerBlock(nn.Module):
    def __init__(self, args, hidden_dims=[]):
        ...
        self.lscale_head = SigmoidUpdateHead(
            hidden_dims[2], hidden_dim=256, output_dim=1
        )
        self.conf_head = SigmoidUpdateHead(hidden_dims[2], hidden_dim=256, output_dim=1)
        self.gscale_gshift_head = ScaleShiftUpdateHead(
            hidden_dims[2], hidden_dim=256, output_dim=2
        )
    ...
    def forward(
        ...
        delta_gscale, delta_gshift = (
            delta_gscale_gshift[:, 0:1],
            delta_gscale_gshift[:, 1:2],
        )
        delta_confidence = self.conf_head(net[0])
