...
class DoubleFeatureAtt(nn.Module):
    ...
    def forward(self, cv, feat_left, feat_right):
        ...
        feat_att_right = (
            self.feat_att_right(feat_right).permute(0, 1, 3, 2).unsqueeze(4)
        )
        _cv = torch.sigmoid(feat_att_left) * torch.sigmoid(feat_att_right)
        _cv = F.interpolate(
            _cv,
            size=(cv.shape[2], cv.shape[3], cv.shape[4]),
            mode="trilinear",
            align_corners=True,
        )
        cv = _cv * cv
...
class FeatureV3(SubModule):
    def __init__(self):
        ...
        self.deconv32_16 = Conv2x(
            chans[4], chans[3], deconv=True, concat=True
        )  # 576 -> 48*2=96
        self.deconv16_8 = Conv2x(
            chans[3] * 2, chans[2], deconv=True, concat=True
        )  # 48*2 -> 24*2=48
        self.deconv8_4 = Conv2x(
            chans[2] * 2, chans[1], deconv=True, concat=True
        )  # 24*2 -> 16*2=32
        self.conv4 = BasicConv(
            chans[1] * 2, chans[1] * 2, kernel_size=3, stride=1, padding=1
        )  # 16*2 -> 16*2=32
...
class FeatureV4(SubModule):
    def __init__(self, img_channels=3, n_downsample=2):
        ...
        if n_downsample not in [0, 1, 2, 3]:
            raise ValueError(
                "n_downsample must be in [0, 1, 2, 3] -- i.e, x1 x2 x4 x8 sub-sampling"
            )
        ...
        self.mapping_conv = nn.Conv2d(
            img_channels, 3, kernel_size=1, stride=1, padding=0
        )
        ...
        if n_downsample < 2:
            self.deconv4_2 = Conv2x(
                chans[1] * 2, chans[0], deconv=True, concat=True
            )  # 32*2 -> 32*2=64

        _i = n_downsample_mapping[n_downsample]
        self.final_conv = BasicConv(
            chans[_i] * 2, chans[_i] * 2, kernel_size=3, stride=1, padding=1
        )
