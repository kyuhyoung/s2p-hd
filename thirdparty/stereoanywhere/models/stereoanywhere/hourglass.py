...
class Hourglass(nn.Module):
    def __init__(
        ...
        act_fn="lrelu",
        n_downsample=2,
        feature_channels=[64, 64, 64, 128, 192, 128],
        att_kernel_size=1,
        att_stride=1,
        att_padding=0,
    ):
        ...
        for i in range(self.number_of_scales - 2):
            agg_in_channels = in_channels * (
                2 * (self.number_of_scales - i - 1)
            ) + in_channels * (2 * (self.number_of_scales - i - 2))
        ...
        self.feature_atts_up = nn.ModuleList(
            [
                DoubleFeatureAtt(
                    in_channels * (2 * (self.number_of_scales - i - 1)),
                    self.feature_channels[self.number_of_scales - i - 1],
                    kernel_size=att_kernel_size,
                    stride=att_stride,
                    padding=att_padding,
                )
                for i in range(1, self.number_of_scales - 1)
            ]
        )

        self.final_feature_atts_up = DoubleFeatureAtt(
            out_channels,
    ...
    def forward(self, x, features_left, features_right):
        ...
        for i in range(self.number_of_scales - 2):
            ...
            x_up = F.interpolate(
                downsampled_features[self.number_of_scales - 2 - i],
                (_up_shape[2], _up_shape[3], _up_shape[4]),
                mode="trilinear",
                align_corners=True,
            )
            x = torch.cat(
                (x_up, downsampled_features[self.number_of_scales - 3 - i]), dim=1
            )
            x = self.agg_layers[i](x)
            x = self.feature_atts_up[i](
                x,
                features_left[self.number_of_scales - 2 - i],
                features_right[self.number_of_scales - 2 - i],
            )
