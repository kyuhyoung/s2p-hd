...
def ransac_depth_scale_shift(
    mde, reference_depths, residual_threshold=1.0, depth_threshold=0.1
):
    """
    ...
    for b in range(B):
        ...
        ransac = RANSACRegressor(
            LinearRegression(), residual_threshold=residual_threshold
        )
        ...
        scale_and_shift[b, 0] = torch.from_numpy(np.array(scale, dtype=np.float32)).to(
            device
        )
        scale_and_shift[b, 1] = torch.from_numpy(np.array(shift, dtype=np.float32)).to(
            device
        )

    return scale_and_shift[:, 0].reshape(B, 1, 1, 1), scale_and_shift[:, 1].reshape(
        B, 1, 1, 1
    )
...
def sgm_opencv(imgL, imgR, maxdisp=192, p_factor=7, w_size=16, interpolate=True):
    ...
    padded_left = cv2.copyMakeBorder(
        imgL, 0, 0, num_disp, 0, cv2.BORDER_CONSTANT, None, 0
    )
    padded_right = cv2.copyMakeBorder(
        imgR, 0, 0, num_disp, 0, cv2.BORDER_CONSTANT, None, 0
    )
...
def color_error_image_kitti(errors, scale=1, mask=None, BGR=True, dilation=1):
    ...
    for col in _color_map_errors_kitti:
        col_mask = np.logical_and(
            errors_flat >= col[0] / scale, errors_flat <= col[1] / scale
        )
    ...
    colored_errors = colored_errors_flat.reshape(
        errors.shape[0], errors.shape[1], 3
    ).astype(np.uint8)
...
def guided_visualize(disp, gt, valid, scale=1, dilation=7):
    ...
    bad1[error > 1.0, :] = (49, 54, 149)
    bad1[error <= 1.0, :] = (165, 0, 38)
    bad1[valid == 0, :] = (0, 0, 0)

    bad2 = np.zeros((H, W, 3), dtype=np.uint8)
    bad2[error > 2.0, :] = (49, 54, 149)
    bad2[error <= 2.0, :] = (165, 0, 38)
    bad2[valid == 0, :] = (0, 0, 0)

    bad3 = np.zeros((H, W, 3), dtype=np.uint8)
    bad3[error > 3.0, :] = (49, 54, 149)
    bad3[error <= 3.0, :] = (165, 0, 38)
    bad3[valid == 0, :] = (0, 0, 0)

    bad4 = np.zeros((H, W, 3), dtype=np.uint8)
    bad4[error > 4.0, :] = (49, 54, 149)
    bad4[error <= 4.0, :] = (165, 0, 38)
    bad4[valid == 0, :] = (0, 0, 0)
...
def sample_hints(hints, probability=0.05):
    ...
    valid_hints = hints > 0
    valid_hints = (valid_hints & (np.random.rand(*hints.shape) < probability)).astype(
        np.float32
    )
...
def image_to_coordinates_array(grayscale_image):
    ...
    x_coords = (
        x_coords.unsqueeze(0).repeat(B, 1, 1).reshape(B, -1).to(grayscale_image.device)
    )
    y_coords = (
        y_coords.unsqueeze(0).repeat(B, 1, 1).reshape(B, -1).to(grayscale_image.device)
    )
...
def coordinates_array_to_image(coordinates_array, H, W, scale=1):
    ...
    image = torch.zeros((B, 1, H, W), dtype=coordinates_array[:, 2].dtype).to(
        coordinates_array.device
    )
...
def estimate_normals_from_disparity(disparity_map):
    ...
    dx = F.pad(
        disparity_map[:, :, :, 1:] - disparity_map[:, :, :, :-1],
        (1, 0, 0, 0),
        mode="replicate",
    )
    dy = F.pad(
...
def weighted_lsq(mono, stereo, conf, min_disp=0.5):
    ...
    return scale_shift[:, 0:1].reshape(B, 1, 1, 1), scale_shift[:, 1:2].reshape(
        B, 1, 1, 1
    )
