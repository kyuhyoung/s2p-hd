...
def demo(args):
    ...
    with torch.no_grad():
        left_images = sorted(
            glob.glob(
                os.path.join(
                    "/data2/cjd/StereoDatasets/middlebury/",
                    "MiddEval3",
                    "testF",
                    "*/im0.png",
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="/data2/cjd/mono_fusion/checkpoints/middlebury.pth",
    )
    parser.add_argument(
        "--save_numpy", action="store_true", help="save output as numpy arrays"
    )
    parser.add_argument(
        "-l", "--left_imgs", help="path to all first (left) frames", default=None
    )
    parser.add_argument(
        "-r", "--right_imgs", help="path to all second (right) frames", default=None
    )
    parser.add_argument(
        "--output_directory",
        help="directory to save output",
        default="./test_output/middlebury",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max_disp", type=int, default=768, help="max disp of geometry encoding volume"
    )
