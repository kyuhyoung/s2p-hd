...
def validate_eth3d(model, iters=32, mixed_prec=False):
    ...
    for val_id in range(len(val_dataset)):
        (imageL_file, imageR_file, GT_file), image1, image2, flow_gt, valid_gt = (
            val_dataset[val_id]
        )
...
def validate_vkitti(model, iters=32, mixed_prec=False):
    ...
    print(
        f"Validation VKITTI: EPE {epe}, D1 {d1}, {format(1 / avg_runtime, '.2f')}-FPS ({format(avg_runtime, '.3f')}s)"
    )
...
def validate_driving(model, iters=32, mixed_prec=False):
    ...
    for val_id in range(len(val_dataset)):
        ...
        if val_id < 9 or (val_id + 1) % 10 == 0:
            logging.info(
                f"Driving Iter {val_id+1} out of {len(val_dataset)}. EPE {round(image_epe,4)} D1 {round(image_out,4)}. Runtime: {format(end-start, '.3f')}s ({format(1/(end-start), '.2f')}-FPS)"
            )
        epe_list.append(epe_flattened[val].mean().item())
        out_list.append(out[val].cpu().numpy())
    ...
    print(
        f"Validation DrivingStereo: EPE {epe}, bad1 {bad_1}, bad2 {bad_2}, bad3 {d1}, {format(1/avg_runtime, '.2f')}-FPS ({format(avg_runtime, '.3f')}s)"
    )
...
def validate_middlebury(model, iters=32, split="F", mixed_prec=False):
    ...
    for val_id in range(len(val_dataset)):
        ...
        occ_mask = Image.open(imageL_file.replace("im0.png", "mask0nocc.png")).convert(
            "L"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="/data2/cjd/mono_fusion/checkpoints/sceneflow.pth",
    )
    ...
    parser.add_argument(
        ...
        default="sceneflow",
        choices=["eth3d", "kitti", "sceneflow", "vkitti", "driving"]
