...
def demo(args):
    ...
    videoWrite = cv2.VideoWriter(
        "./monster_demo.mp4", cv2.VideoWriter_fourcc(*"mp4v"), 10, (1241, 752)
    )
    ...
    with torch.no_grad():
        ...
        for imfile1, imfile2 in tqdm(list(zip(left_images, right_images))):
            ...
            file_stem = os.path.join(output_directory, imfile1.split("/")[-1]).replace(
                ".png", ""
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="/data2/cjd/mono_fusion/checkpoints/mix_all.pth",
    )
    ...
    parser.add_argument(
        ...
        default="/data/cjd/kitti_odometry/dataset/sequences/00/image_2/*.png",
    )
    parser.add_argument(
        "-r",
        "--right_imgs",
        help="path to all second (right) frames",
        default="/data/cjd/kitti_odometry/dataset/sequences/00/image_3/*.png",
    )

    parser.add_argument(
        "--output_directory", help="directory to save output", default="kitti"
    )
    parser.add_argument(
