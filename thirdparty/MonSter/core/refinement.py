...
class Attention_HourglassModel(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv3a = BasicConv_now(
            64, 96, kernel_size=3, stride=2, dilation=2, padding=2
        )
        self.conv4a = BasicConv_now(
            96, 128, kernel_size=3, stride=2, dilation=2, padding=2
        )
...
class REMP(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_start = BasicConv_now(
            64, channel, kernel_size=3, padding=2, dilation=2
        )
    ...
    def forward(self, disp_mono, disp_stereo, left_img, right_img):
        ...
        x = torch.cat(
            (ref_flaw_mono, disp_fea_mono, ref_flaw_stereo, disp_fea_stereo), dim=1
        )  # [B, 64, H, W]
...
class CS(nn.Module):
    def __init__(self):
        ...
        self.conv1_scale = nn.Sequential(
            conv2d(1, 16), nn.Hardtanh(min_val=0.0, max_val=5.0)
        )
        ...
        self.final_conv = nn.Sequential(
            nn.Conv2d(channel, 1, 3, 1, 1), nn.Hardtanh(min_val=0.0, max_val=3.0)
        )
...
class fusion_mono(nn.Module):
    def __init__(self):
        ...
        self.RefinementBlock = Simple_UNet_8x(in_channels=channel)  # , in_channels
        self.final_conv = nn.Sequential(
            nn.Conv2d(channel, 1, 3, 1, 1), nn.ReLU(inplace=True)
        )
