...
class SceneFlowDatasets(StereoDataset):
    def __init__(
        self,
        aug_params=None,
        root="/data2/cjd/StereoDatasets/sceneflow",
    ...
    def _add_things(self, split="TRAIN"):
        ...
        right_images = sorted(
            glob(osp.join(root, self.dstype, split, "*/*/right/*.png"))
        )
...
class KITTI(StereoDataset):
    def __init__(
        self,
        aug_params=None,
        root="/data2/cjd/StereoDatasets/kitti/2015",
        image_set="training",
    ):
...
class VKITTI2(StereoDataset):
    def __init__(self, aug_params=None, root="/data/cjd/stereo_dataset/vkitti2/"):
        super(VKITTI2, self).__init__(
            aug_params, sparse=True, reader=frame_utils.readDispVKITTI2
        )
        assert os.path.exists(root)

        image1_list = sorted(
            glob(os.path.join(root, "Scene*/*/frames/rgb/Camera_0/rgb*.jpg"))
        )
        image2_list = sorted(
            glob(os.path.join(root, "Scene*/*/frames/rgb/Camera_1/rgb*.jpg"))
        )
        disp_list = sorted(
            glob(os.path.join(root, "Scene*/*/frames/depth/Camera_0/depth*.png"))
        )
...
class Middlebury(StereoDataset):
    def __init__(
        self,
        aug_params=None,
        root="/data2/cjd/StereoDatasets/middlebury",
        split="2014",
        resolution="F",
    ):
        ...
        if split == "2005":
            ...
            for scene in scenes:
                self.image_list += [
                    [str(scene / "view1.png"), str(scene / "view5.png")]
                ]
                ...
                for illum in ["1", "2", "3"]:
                    for exp in ["0", "1", "2"]:
                        self.image_list += [
                            [
                                str(scene / f"Illum{illum}/Exp{exp}/view1.png"),
            ...
            for scene in scenes:
                ...
                for s in ["0", "1", "2", "3"]:
                    if os.path.exists(str(scene / f"ambient/L0/im0e{s}.png")):
                        self.image_list += [
                            [
                                str(scene / f"ambient/L0/im0e{s}.png"),
                                str(scene / f"ambient/L0/im1e{s}.png"),
                            ]
                        ]
                        self.disparity_list += [str(scene / "disp0.pfm")]
        else:
            image1_list = sorted(
                glob(
                    os.path.join(
                        root, "MiddEval3", f"training{resolution}", "*/im0.png"
                    )
                )
            )
            image2_list = sorted(
                glob(
                    os.path.join(
                        root, "MiddEval3", f"training{resolution}", "*/im1.png"
                    )
                )
            )
            disp_list = sorted(
                glob(
                    os.path.join(
                        root, "MiddEval3", f"training{resolution}", "*/disp0GT.pfm"
                    )
                )
            )
...
class CREStereoDataset(StereoDataset):
    def __init__(self, aug_params=None, root="/data2/cjd/StereoDatasets/crestereo"):
        super(CREStereoDataset, self).__init__(
            aug_params, reader=frame_utils.readDispCREStereo
        )
        assert os.path.exists(root)
...
class InStereo2K(StereoDataset):
    def __init__(self, aug_params=None, root="/data2/cjd/data_wxq/instereo2k"):
        super(InStereo2K, self).__init__(
            aug_params, sparse=True, reader=frame_utils.readDispInStereo2K
        )
        assert os.path.exists(root)

        image1_list = sorted(
            glob(root + "/train/*/*/left.png") + glob(root + "/test/*/left.png")
        )
        image2_list = sorted(
            glob(root + "/train/*/*/right.png") + glob(root + "/test/*/right.png")
        )
        disp_list = sorted(
            glob(root + "/train/*/*/left_disp.png")
            + glob(root + "/test/*/left_disp.png")
        )
...
class DrivingStereo(StereoDataset):
    def __init__(
        self,
        aug_params=None,
        root="/data2/cjd/StereoDatasets/drivingstereo/",
        image_set="rainy",
    ):
        ...
        assert os.path.exists(root)
        image1_list = sorted(
            glob(os.path.join(root, image_set, "left-image-half-size/*.jpg"))
        )
        image2_list = sorted(
            glob(os.path.join(root, image_set, "right-image-half-size/*.jpg"))
        )
        disp_list = sorted(
            glob(os.path.join(root, image_set, "disparity-map-half-size/*.png"))
        )
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        if dataset_name == "sceneflow":
            ...
            logging.info(
                f"Adding {len(new_dataset)} samples from ETH3D Mixture Dataset"
            )
            ...
            logging.info(
                f"Adding {len(new_dataset)} samples from Middlebury Mixture Dataset"
            )
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        image1_np = (
            (image1_np - image1_np.min()) / (image1_np.max() - image1_np.min()) * 255.0
        )
