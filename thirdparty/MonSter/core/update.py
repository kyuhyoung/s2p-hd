    ...
    def forward(
        ...
        if iter04:
            motion_features = self.encoder(
                disp, corr, flaw_stereo, disp_mono, corr_mono, flaw_mono
            )
            if self.args.n_gru_layers > 1:
                net[0] = self.gru04(
                    net[0], *(inp[0]), motion_features, interp(net[1], net[0])
...
class BasicMultiUpdateBlock_2(nn.Module):
    ...
    def forward(
        ...
        if iter04:
            motion_features = self.encoder(
                disp, corr, flaw_stereo, disp_mono, corr_mono, flaw_mono, confidence
            )
            if self.args.n_gru_layers > 1:
                net[0] = self.gru04(
                    net[0], *(inp[0]), motion_features, interp(net[1], net[0])
...
class BasicMotionEncoder_mix_conf(nn.Module):
    ...
    def forward(
        ...
        cor = F.relu(self.convc2(cor))
        cor_mono = F.relu(
            self.convc1_mono(torch.cat([corr_mono, flaw_mono, conf_mono_], dim=1))
        )
        cor_mono = F.relu(self.convc2_mono(cor_mono))
        ...
        return torch.cat(
            [out, disp, conf_stereo, out_mono, disp_mono, conf_mono], dim=1
        )
