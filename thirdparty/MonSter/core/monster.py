...
def compute_scale_shift(monocular_depth, gt_depth, mask=None):
    ...
    if mask is None:
        mask = (
            (gt_depth > 0)
            & (monocular_depth > 1e-2)
            & (monocular_depth > threshold_10_percent)
        )
    ...
    X = torch.stack(
        [monocular_depth_flat, torch.ones_like(monocular_depth_flat)], dim=1
    )
    y = gt_depth_flat
...
class Feat_transfer_cnet(nn.Module):
    def __init__(self, dim_list, output_dim):
        ...
        self.res_16x = nn.Conv2d(
            dim_list[0] + 192, output_dim, kernel_size=3, padding=1, stride=1
        )
        self.res_8x = nn.Conv2d(
            dim_list[0] + 96, output_dim, kernel_size=3, padding=1, stride=1
        )
        self.res_4x = nn.Conv2d(
            dim_list[0] + 48, output_dim, kernel_size=3, padding=1, stride=1
        )
...
class Feat_transfer(nn.Module):
    def __init__(self, dim_list):
        ...
        self.conv16x = nn.Sequential(
            nn.Conv2d(
                in_channels=int(192 + dim_list[0]),
                out_channels=192,
                kernel_size=5,
                stride=1,
                padding=2,
        ...
        self.conv_up_32x = nn.ConvTranspose2d(
            160, 192, kernel_size=3, padding=1, output_padding=1, stride=2, bias=False
        )
        self.conv_up_16x = nn.ConvTranspose2d(
            192, 64, kernel_size=3, padding=1, output_padding=1, stride=2, bias=False
        )
        self.conv_up_8x = nn.ConvTranspose2d(
            64, 48, kernel_size=3, padding=1, output_padding=1, stride=2, bias=False
        )
    ...
    def forward(self, features):
        ...
        feat_16x = self.conv16x(
            torch.cat((features[2], feat_32x_up), 1)
        ) + self.res_16x(features[2])
        feat_16x_up = self.conv_up_16x(feat_16x)
        feat_8x = self.conv8x(torch.cat((features[1], feat_16x_up), 1)) + self.res_8x(
            features[1]
        )
        feat_8x_up = self.conv_up_8x(feat_8x)
        feat_4x = self.conv4x(torch.cat((features[0], feat_8x_up), 1)) + self.res_4x(
            features[0]
        )
...
class Monster(nn.Module):
    def __init__(self, args):
        ...
        self.feat_transfer_cnet = Feat_transfer_cnet(
            dim_list, output_dim=args.hidden_dims[0]
        )
        ...
        depth_anything_decoder = DepthAnythingV2_decoder(
            **mono_model_configs[args.encoder]
        )
        ...
        self.update_block_mix_stereo = BasicMultiUpdateBlock_mix2(
            self.args, hidden_dims=args.hidden_dims
        )
        self.update_block_mix_mono = BasicMultiUpdateBlock_mix2(
            self.args, hidden_dims=args.hidden_dims
        )
    ...
    def infer_mono(self, image1, image2):
        ...
        resize_image1 = F.interpolate(
            image1, scale_factor=14 / 16, mode="bilinear", align_corners=True
        )
        resize_image2 = F.interpolate(
            image2, scale_factor=14 / 16, mode="bilinear", align_corners=True
        )
        ...
        depth_mono = F.interpolate(
            depth_mono,
            size=(height_ori, width_ori),
            mode="bilinear",
            align_corners=False,
        )
        features_left_4x, features_left_8x, features_left_16x, features_left_32x = (
            self.feat_decoder(features_left_encoder, patch_h, patch_w)
        )
        features_right_4x, features_right_8x, features_right_16x, features_right_32x = (
            self.feat_decoder(features_right_encoder, patch_h, patch_w)
        )
    ...
    #             m.eval()
    #             # Optional: also freeze affine params so grads don’t flow
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float32):
            depth_mono, features_mono_left, features_mono_right = self.infer_mono(
                image1, image2
            )

        scale_factor = 0.25
        size = (
            int(depth_mono.shape[-2] * scale_factor),
            int(depth_mono.shape[-1] * scale_factor),
        )

        disp_mono_4x = F.interpolate(
            depth_mono, size=size, mode="bilinear", align_corners=False
        )
        ...
        for itr in range(iters):
            ...
            if itr > int(iters - 8):
                if itr == int(iters - 7):
                    ...
                    for i in range(bs):
                        with torch.autocast(device_type="cuda", dtype=torch.float32):
                            scale, shift = compute_scale_shift(
                                disp_mono_4x[i].clone().squeeze(1).to(torch.float32),
                                disp[i].clone().squeeze(1).to(torch.float32),
                            )
                        disp_mono_4x[i] = scale * disp_mono_4x[i] + shift

                warped_right_mono = disp_warp(
                    features_right[0], disp_mono_4x.clone().to(features_right[0].dtype)
                )[0]
                flaw_mono = warped_right_mono - features_left[0]

                warped_right_stereo = disp_warp(
                    features_right[0], disp.clone().to(features_right[0].dtype)
                )[0]
            ...
            if itr <= int(iters - 8):
                ...
                net_list_mono, mask_feat_4_mono, delta_disp_mono = (
                    self.update_block_mix_mono(
                        net_list_mono,
                        inp_list,
                        flaw_mono,
                        disp_mono_4x,
                ...
                disp_mono_4x_up = self.upsample_disp(
                    disp_mono_4x, mask_feat_4_mono, stem_2x
                )
