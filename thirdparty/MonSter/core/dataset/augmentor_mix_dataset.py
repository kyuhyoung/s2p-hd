...
class Augmentor:
    def __init__(
        ...
        self.carlib_auger = OpticShiftAugmentor(
            height=1408, width=1280, fov_h=185, fov_w=120, maxDegree=1
        )
    ...
    def __call__(
        ...
        if error is not None:
            ...
            disp_mask = (
                (left_disp < float(self.max_disp / resize_scale))
                & (left_disp > 0)
                & error_mask
            )
        else:
            disp_mask = (left_disp < float(self.max_disp / resize_scale)) & (
                left_disp > 0
            )
...
class OpticShiftAugmentor:
    def __init__(self, height, width, fov_h=360, fov_w=180, maxDegree=1):
        ...
        theta_1_map = (
            np.array([theta_1_range for i in range(self.width)]).astype(np.float32).T
        )
        ...
        phi_1_map = np.array([phi_1_range for j in range(self.height)]).astype(
            np.float32
        )
    ...
    def shift(self, rightImg, rng, sigma):
        ...
        theta_2_map = np.arctan2(X_rot[:, :, 1, 0], X_rot[:, :, 2, 0]).astype(
            np.float32
        )
        phi_2_map = np.arcsin(np.clip(X_rot[:, :, 0, 0] / r_2, -1, 1)).astype(
            np.float32
        )
        ...
        # remap
        right_shift = cv2.remap(
            rightImg, LUT_x, LUT_y, cv2.INTER_LINEAR, borderMode=cv2.BORDER_REPLICATE
        )
