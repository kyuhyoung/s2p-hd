    ...
    def __init__(
        ...
        for i in range(len(data_path["image"])):
            ...
            if (
                ...
                items = [
                    line.strip().split(",") for line in open(filelist).readlines()[1:]
                ]
            else:
                items = [
                    line.strip().split(" ") for line in open(filelist).readlines()[:]
                ]
                ...
                items = [
                    {
                        ...
                        "right_disp": (
                            None
                            if "right_disp" not in field
                            else item[field.index("right_disp")]
                        ),
                        "seg_sky": (
                            None
                            if "seg_sky" not in field
                            else item[field.index("seg_sky")]
                        ),
                        "seg_wire": (
                            None
                            if "seg_wire" not in field
                            else item[field.index("seg_wire")]
                        ),
                    }
                    for item in items
                ]
        ...
        if "airsim" in dataset_name:
            ...
            if "far" in dataset_name:
                ...
                if "3d_recon" in dataset_name or "lidar" in dataset_name:
                    ...
                    error = (
                        disps["error"].astype(np.float32) if "error" in disps else None
                    )
            ...
            try:
                ...
                if "realistic" in dataset_name:
                    ...
                    if "far" not in dataset_name:
                        ...
                        right_disp = (
                            None
                            if item["right_disp"] == None
                            else os.path.join(disp_root_path, item["right_disp"])
                        )
                        seg_sky = (
                            None
                            if item["seg_sky"] == None
                            else os.path.join(seg_root_path, item["seg_sky"])
                        )
                        seg_wire = (
                            None
                            if item["seg_wire"] == None
                            else os.path.join(seg_root_path, item["seg_wire"])
                        )
                ...
                assert os.path.exists(
                    left
                ), f"dataset: {dataset_name}, root: {image_root_path}, left_path: {left}"
                ...
                if "far" not in dataset_name:
                    left_disp, left_error = self.get_disp(
                        left_disp, left_img.shape[0], left_img.shape[1], dataset_name
                    )

                    right_disp, right_error = right_disp, (
                        None
                        if right_disp == None
                        else self.get_disp(
                            right_disp,
                    ...
                    left_disp, left_error = self.get_disp(
                        "", left_img.shape[0], left_img.shape[1], dataset_name
                    )
                    right_disp, right_error = "", None

                if dataset_name in ["TartanAir", "vkitti2", "sintel"]:
                    left_img = cv2.resize(
                        left_img, None, fx=1.5, fy=1.5, interpolation=cv2.INTER_LINEAR
                    )
                    right_img = cv2.resize(
                        right_img, None, fx=1.5, fy=1.5, interpolation=cv2.INTER_LINEAR
                    )
                    left_disp = (
                        cv2.resize(
                            left_disp,
                ...
                if (
                    ...
                    and self.rng.binomial(1, 0.5) == 1
                    and isinstance(right_disp, np.ndarray)
                ):
                ...
                left_disp[np.isnan(left_disp)] = 0.0

                if (
                    self.rng.binomial(1, 0.5) == 1
                    and isinstance(right_disp, np.ndarray)
                    and self.camera_type != "pinhole"
                ):
                ...
                if self.augmentor is not None and (
                    left_img.shape != (3, self.height, self.width)
                    or right_img.shape != (3, self.height, self.width)
                ):
