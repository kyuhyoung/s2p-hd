    ...
    brightness = A.Compose(
        [
            ...
            A.HueSaturationValue(
                hue_shift_limit=15, sat_shift_limit=10, val_shift_limit=15
            ),  # 色调
        ],
        p=1,
    )
...
def chromatic_augmentation_v3(l_img, r_img, rng, motion_blur_aug, albumentations_aug):
    ...
    if albumentations_aug and rng.binomial(1, 0.5):
        ...
        r_img = random_hue_saturation(
            r_img, hue_shift * noise_hue, sat_shift * noise_sat, val_shift * noise_val
        )
...
def mask_ge(shape, rng, weights=[0.5, 0.5]):
    ...
    if mask_model == "random":
        ...
        blur = cv2.GaussianBlur(
            noise, (0, 0), sigmaX=3, sigmaY=3, borderType=random_borderType
        )  # cv2.BORDER_CONSTANT
        ...
        result = cv2.GaussianBlur(
            open_result, (5, 5), sigmaX=0, sigmaY=0, borderType=cv2.BORDER_DEFAULT
        )
    else:
        width = 100
        height = int(width * shape[0] / shape[1])
        ellipse_height = (
            rng.randint(low=int(width / 4), high=int(width / 2)) * 2 + 1
        )  # Choose an odd kernel height between 3 and 29
        ellipse_width = (
            rng.randint(low=int(width / 4), high=int(width / 2)) * 2 + 1
        )  # Choose an odd kernel width between 3 and 29
        blur_pos = rng.randint(low=0, high=height), rng.randint(
            low=0, high=width
        ...
        cv2.ellipse(
            mask,
            center=(
                blur_pos[1] + ellipse_width // 2,
                blur_pos[0] + ellipse_height // 2,
            ),
        ...
        # 高斯模糊
        result = cv2.GaussianBlur(
            open_result, (21, 21), sigmaX=0, sigmaY=0, borderType=cv2.BORDER_DEFAULT
        )
