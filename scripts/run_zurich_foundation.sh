#!/bin/bash
# Zurich (PNEO 4뷰 cross-date) FoundationStereo DSM. 호스트 단발 docker.
...
LOGFILE="${SCRIPT_DIR}/run_zurich_foundation.log"; : > "$LOGFILE"
exec > >(stdbuf -oL tee "$LOGFILE") 2>&1
GREEN='\033[0;32m'; YELLOW='\033[1;33m'; RED='\033[0;31m'; NC='\033[0m'
echo "========== $(date '+%Y-%m-%d %H:%M:%S') Zurich Foundation =========="
...
cfg_host="${DIR_DATA}/${REL}/foundation_config.json"; cfg_docker="/data/${REL}/foundation_config.json"
out_host="${DIR_DATA}/${REL}/2_s2p-hd/output_foundation"; pan_host="${DIR_DATA}/${REL}/1_sat-ba/pan"
MODEL_WARM=1   # dl_stereo 매처는 warming
EXCLUDE=${EXCLUDE:-PNEO4}   # DSM에서 제외할 뷰(파일명 부분문자열). PNEO4=여름 7개월차 → 20m bias라 제외. ""면 전체.

echo -e "${GREEN}  Zurich (PNEO 4뷰 cross-date) Foundation | GPUs=${ID_GPUS}${NC}"
