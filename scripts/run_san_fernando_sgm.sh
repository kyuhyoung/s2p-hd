#!/bin/bash
# =============================================================
# San Fernando classic SGM (stereosgm_gpu, PAN) DSM
#   config: sgm_north_config.json   출력: output_sgm_north
#   foundation(deep)와 완전 분리 — 서로 안 건드림.
#   sudo 없이 docker 실행(소켓 접근 가능 시), + dabeeo fill.
# =============================================================
set -uo pipefail
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LOGFILE="${SCRIPT_DIR}/run_san_fernando_sgm.log"; : > "$LOGFILE"
exec > >(stdbuf -oL tee "$LOGFILE") 2>&1
echo "===== $(date '+%Y-%m-%d %H:%M:%S')  classic SGM ====="
...
cfg=/data/satellite/argentina/san_fernando/sgm_north_config.json
out=$DIR/satellite/argentina/san_fernando/2_s2p-hd/output_sgm_north
ID_GPUS=${ID_GPUS:-0,1,2,3,4,5,6,7}
FILL_MODE=${FILL_MODE:-avg}

$SUDO rm -rf "$out"
INNER="set -e
pip3 install --quiet --root-user-action=ignore -e /workspace 2>&1|tail -1
echo '=== s2p stereosgm_gpu (PAN, north) ==='
stdbuf -oL s2p ${cfg}
echo '=== done ==='"
...
rc=$?
[ -f "${out}/dsm.tif" ] || { echo "SGM FAILED (exit=$rc)"; exit $rc; }
python3 -c "
import rasterio,numpy as np
with rasterio.open('${out}/dsm.tif') as r:
 a=r.read(1).astype(float);nd=r.nodata;v=np.isfinite(a)&(a!=nd if nd else True)
 print(f'SGM DSM: {r.width}x{r.height}, valid {100*v.sum()/a.size:.1f}%, z {np.nanpercentile(a[v],[1,50,99]).round(1) if v.sum() else None}')
"
# dabeeo fill
