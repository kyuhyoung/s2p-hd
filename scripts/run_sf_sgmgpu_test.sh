...
LOGFILE="${SCRIPT_DIR}/run_sf_sgmgpu_test.log"
...
cfg_docker="/data/satellite/argentina/san_fernando/sgmgpu_test_config.json"
out_host="${DIR_DATA}/satellite/argentina/san_fernando/2_s2p-hd/output_sgmgpu_test"
ID_GPU=${ID_GPU:-7}

sudo rm -rf "$out_host"
INNER="set -e
pip3 install --quiet --root-user-action=ignore -e /workspace 2>&1 | tail -1
echo '=== s2p stereosgm_gpu ==='
stdbuf -oL s2p ${cfg_docker}
echo '=== done ==='
"
stdbuf -oL sudo docker run --rm --user "$(id -u):$(id -g)" \
    --gpus "device=${ID_GPU}" --shm-size=64g --net=host -e HOME=/tmp \
    -v "${DIR_DATA}":/data -v "${S2P_HD_REPO}":/workspace \
    --entrypoint bash s2p-hd-dl:latest -c "${INNER}"
rc=$?
echo ""
if [ -f "${out_host}/dsm.tif" ]; then
    echo "SGMGPU DSM done:"
    python3 -c "
