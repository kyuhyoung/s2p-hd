...
# 입력: pan_aoi/{522,544,554}.tif (PAN 1밴드 직접 — RGB 추출 없음), foundation_real_config.json
...
LOGFILE="${SCRIPT_DIR}/run_sf_foundation_real.log"
: > "$LOGFILE"
exec > >(stdbuf -oL tee "$LOGFILE") 2>&1
GREEN='\033[0;32m'; YELLOW='\033[1;33m'; RED='\033[0;31m'; NC='\033[0m'
echo "========== $(date '+%Y-%m-%d %H:%M:%S') =========="

# Use docker without sudo when the daemon socket is reachable (rootless /
# docker group); otherwise fall back to sudo with a keep-alive. SUDO is the
# prefix used for all privileged commands below.
if docker ps >/dev/null 2>&1; then
    SUDO=""
    echo "docker: sudo 없이 실행 (socket 접근 가능)"
else
    SUDO="sudo"
    sudo -v
...
cfg_host="${DIR_DATA}/${REL}/foundation_real_config.json"
cfg_docker="/data/${REL}/foundation_real_config.json"
out_host="${DIR_DATA}/${REL}/2_s2p-hd/output_foundation_real"
